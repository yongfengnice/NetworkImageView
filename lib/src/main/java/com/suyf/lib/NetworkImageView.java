...
import com.suyf.lib.ImageLoader.ImageContainer;
import com.suyf.lib.ImageLoader.ImageListener;
...
public class NetworkImageView extends ImageView {
    ...
    mImageContainer =
        mImageLoader.get(
            ...
            new ImageListener() {
              ...
              @Override
              public void onDrawable(final ImageContainer response, boolean isImmediate) {
                if (isImmediate && isInLayoutPass) {
                  post(
                      new Runnable() {
                        @Override
                        public void run() {
                          onDrawable(response, /* isImmediate= */ false);
                        }
                      });
                  return;
                }
                if (response.getDrawable() != null) {
                  setContentDrawable(response.getDrawable());
                } else {
                  setDefaultImageOrNull();
                }
              }
              ...
              public void onResponse(final ImageContainer response, boolean isImmediate) {
                ...
                if (response.getBitmap() != null) {
                  setContentImage(response.getBitmap());
                } else {
                  setDefaultImageOrNull();
                }
              }
  ...
  private void setDefaultImageOrNull() {
    ...
    if (mListener != null) {
      mListener.onImageStateChange(getCurrentState());
    }
  }

  private void setErrorImageOrNull() {
    if (mErrorImageId != 0) {
  ...
  private void setContentDrawable(@NonNull Drawable drawable) {
  ...
  private void setContentImage(@NonNull Bitmap bitmap) {
  ...
  public void setOnImageStateChangeListener(OnImageStateChangeListener listener) {
    this.mListener = listener;
  }

  public interface OnImageStateChangeListener {

    void onImageStateChange(int state);
  }
}
